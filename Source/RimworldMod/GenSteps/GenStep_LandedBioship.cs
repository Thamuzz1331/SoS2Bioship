...
namespace RimWorld
{
    class GenStep_LandedBioship : GenStep_Scatterer
    {
        ...
        protected override void ScatterAt(IntVec3 c, Map map, GenStepParams stepparams, int stackCount = 1)
        {
            ...
            try
            {
                ShipCombatManager.GenerateShip(DefDatabase<EnemyShipDef>.AllDefs.Where(def=>def.spaceSite).RandomElement(), map, null, Faction.OfAncients, null, out core, true, true);
            }
            catch(Exception e)
            {
            ...
            {
                if (building.Spawned && building.def.CostList?.Where(count => count.thingDef == ThingDefOf.ComponentSpacer).Count() > 0)
                    toKill.Add(building);
